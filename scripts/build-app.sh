...
# Assert an interpreter tree has every backend dependency installed. Used at
# [6b/7] on the interpreter INSIDE the bundle, so an .app missing a dep can
# never be signed. The same check on build/py lives in
# scripts/preflight-vendored-python.sh, which also owns the staleness rules and
# which `cargo tauri build` runs by itself (tauri.conf.json beforeBuildCommand).
check_vendored_py() {
...
# Belt-and-braces inside this script — [2/7] above always re-vendors, so the
# staleness half cannot fire in a full run. The preflight exists as its own
# script because the incident's path was a hand-run `cargo tauri build`, which
# now runs it too via tauri.conf.json's beforeBuildCommand.
bash "$ROOT/scripts/preflight-vendored-python.sh"
