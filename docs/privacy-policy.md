...
- Access is **read-only** and is granted only after you either (a) sign in to your school's Moodle site in your browser, using Moodle's own mobile-app sign-in handoff — you authenticate on your school's own pages (for NC State WolfWare, Shibboleth SSO), and Moodle passes only web-service tokens back to the app, of which only the access token (a `wstoken`) is stored; ScuffedOS uses it read-only — or (b) explicitly paste in an access token you obtain from that site (for WolfWare, from the Security-keys page after signing in). Either way ScuffedOS never sees your school username or password; only the resulting token is stored, server-side, and it is never exposed to the client.
